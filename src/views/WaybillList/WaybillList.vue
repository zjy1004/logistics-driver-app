                  ...
                  <div class="foldingList" v-if="miditem.isUp">
                    <div class="cellList" :key="index" v-for="(innerItem,index) in miditem.childrenWaybill">
                      <flexbox>
                        ...
                        <flexbox-item :span="1/2"><div class="flex-demo"><span class="money">￥{{innerItem.total}}</span> ({{innerItem.receivableFreight}}<span class="status">{{innerItem.settlementMethodName}}</span>+{{innerItem.receivableGoods}})</div></flexbox-item>
...
export default {
  ...
  methods: {
    ...
    searchBtn () {
      ...
      this.listData = []
      this.sourceListData.forEach(item => {
        let tempObj = {...item}
        let tempArr = []
        item.childrenWaybill.forEach(innerItem => {
          if (innerItem.receiveClientName.includes(this.searchValue)) {
            tempArr.push(innerItem)
          }
        })
        tempObj.childrenWaybill = tempArr
          ...
          this.listData = res.data.map(item => {
            item.childrenWaybill && item.childrenWaybill.map(midItem => {
              ...
              midItem.childrenWaybill && midItem.childrenWaybill.map(innerItem => {
                if (innerItem.freightPayer === 1) { // 发货方

                }
                if (innerItem.freightPayer === 2) { // 收货方

                }
                if (innerItem.settlementMethod === 1) {
                  innerItem.total = (innerItem.receivableFreight + innerItem.receivableGoods).toFixed(1)
                  innerItem.settlementMethodName = ''
                } else if (innerItem.settlementMethod === 2) {
                  innerItem.total = (innerItem.receivableGoods).toFixed(1)
                  if (innerItem.freightPayer === 1) { // 发货方

                  } else {
                    innerItem.settlementMethodName = '(月结)'
                  }
                } else if (innerItem.settlementMethod === 3) {
                  innerItem.total = (innerItem.receivableGoods).toFixed(1)
                  if (innerItem.freightPayer === 1) { // 发货方

                  } else {
                    innerItem.settlementMethodName = '(包月)'
                  }
                } else {
                  innerItem.total = (innerItem.receivableFreight + innerItem.receivableGoods).toFixed(1)
                }
