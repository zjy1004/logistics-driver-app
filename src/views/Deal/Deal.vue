                ...
                <ul>
                  <li :key="index" v-for="(boxItem, index) in item.childrenWaybill">
                    ...
                    <div class="right">
                      ...
                      <div class="right-money" v-if="boxItem.freightPayer === 2">
                        ¥
                        <span class="midName">{{boxItem.total}}</span>
                        <span class="rightName">
                        ({{boxItem.receivableFreight}}
                        <span class="rightName" v-if="boxItem.settlementMethod === 2">(月结)</span>
                        <span class="rightName" v-if="boxItem.settlementMethod === 3">(包月)</span>+{{boxItem.receivableGoods}})</span>
                      </div>
                      <div class="right-money" v-if="boxItem.freightPayer === 1">
                        ¥
                        <span class="midName">{{boxItem.total}}</span>
                        <span class="rightName">(0+{{boxItem.receivableGoods}})</span>
                      </div>
                    </div>
                  </li>
                </ul>
...
export default {
  ...
  methods: {
    ...
    // 待收款-选中运单明细
    handleReceiveDetail (type) {
      if (!this.receiveDetail) {
        this.receiveDetailList = [] // 2019-1-10加
        this.receiveListData.map(item => {
          let obj = {}
          ...
          if (hasChildChecked) {
            ...
            checkedItems.forEach(childrenItem => {
              obj.freightPayer = childrenItem.freightPayer
            })
      ...
      if (type === 'paySuccess') {
        this.receiveDetail = false
        this.recIsShowShadow = false
      } else {
      ...
      if (!this.dealDetail) {
        this.dealListData.map(item => {
          ...
          if (hasChildChecked) {
            ...
            checkedItems.forEach(childrenItem => {
              obj1.freightPayer = childrenItem.freightPayer
            })
    ...
    paySuccess () {
      ...
      this.handleReceiveDetail('paySuccess')
      setTimeout(() => {
        this.getRecWayBill()
