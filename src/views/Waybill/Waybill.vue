<template>
   ...
   <div class="waybill">
    ...
    <div class="content">
      <div class="create-waybill" v-if="showCreateWaybill" @click="createWaybill"><div class="add-icon">+</div><div class="text">下单</div></div>
      <!-- 有运单 -->
      <div v-if="waybillListData.length > 0"  class="c-waybill">
        <van-pull-refresh v-model="isLoading" @refresh="waybillRefresh">
          <ul class="c-w-wrap">
            <li class="c-w-item" :key="index" v-for="(item, index) in waybillListData" @click="handleDetail(item)">
              <div class="item-top">
                <div class="name">
              ...
              <div class="item-mid">
                ...
                <div class="item-mid_div money">
                  ...
                  <div class="freightAmount" v-if="item.withdrawStatus !== null">
                    <span class="title">提现状态：</span>
                    <span>{{item.withdrawStatus}}</span>
                  </div>
                </div>
              </div>
              <div class="item-mid" v-if="item.waybillStatus !== '待完善'">
                <div class="item-mid_div money">
                  <div class="loanAmount">
              ...
              <div class="btnGroup" v-if="item.waybillStatus === '待完善'">
                <div class="left">
                  <van-button round type="default" @click.stop="openCancelDialog(item.frontWaybillId)">取消</van-button>
                </div>
                <div class="right">
                  <van-button round type="default" @click.stop="openQRcode(item.frontWaybillId)">开单二维码</van-button>
                  <van-button round type="default" @click.stop="editOrder(item.frontWaybillId)">修改</van-button>
                </div>
              </div>
            </li>
          </ul>
          ...
          <div class="billStatus">
            <h3>运单状态</h3>
              <checker type="checkbox" v-model="billStatus" default-item-class="demo1-item" selected-item-class="demo1-item-selected">
                <div class="checkLine">
                  <checker-item v-if="showCreateWaybill" :value="12">{{ '待完善'}}</checker-item>
    ...
    <van-dialog
      ...
      title="是否确认取消该运单"
      show-cancel-button
      @confirm="deleteWaybill"
      @cancel="cancelWaybill"
    >
    </van-dialog>
   </div>
</template>
...
import { Popup, TransferDom, Datetime, Calendar, Checker, CheckerItem, Qrcode } from 'vux'
import { PullRefresh, Dialog, Button } from 'vant'
import WaybillAjax from '@/api/WayBill/WayBill'

export default {
  components: { 'v-footer-bar': FooterBar, Popup, TransferDom, Datetime, Calendar, Checker, CheckerItem, Qrcode, [PullRefresh.name]: PullRefresh, [Dialog.name]: Dialog, [Button.name]: Button },
  directives: {TransferDom},
  name: 'WaybillList',
  data () {
    return {
      showCreateWaybill: false,
      deleteID: '',
      qrUrl: '',
      telNum: '',
      isIos: true,
      ...
      QRcodeShow: false,
      waybillQueryParam: {
        // createTimeEnd: this.getDateStr(0),
        // createTimeStart: this.getDateStr(0),
        waybillType: 1,
        createTimeEnd: '',
        createTimeStart: '',
        waybillStatusList: []
  ...
  created () {
    ...
    if (userInfo.businessModel === 2 || userInfo.businessModel === 3) {
      this.showCreateWaybill = true
    }
    this.waybillQueryParam.createTimeStart = this.getDateStr(0)
    this.waybillQueryParam.createTimeEnd = this.getDateStr(0)
  ...
  methods: {
    ...
    deleteWaybill (val) {
      WaybillAjax.DeleteTemporaryWaybill({id: this.deleteID}).then(res => {
        if (res.code === 200) {
          this.getWaybill(this.waybillQueryParam)
          this.$vux.toast.show({
            type: 'success',
    ...
    openCancelDialog (val) {
      this.deleteID = val
      this.cancelDialogShow = true
    },
    openQRcode (val) {
      this.QRcodeShow = true
      this.qrUrl = val.toString()
    },
    editOrder (val) {
      this.$router.push({name: 'CreateWaybill', query: {type: 'edit', waybillId: val}})
    },
    createWaybill () {
      this.$router.push({name: 'ChooseCustomers', query: {type: 'insert', goBackType: 'first'}})
    },
    // 运单详情
    handleDetail (val) {
      if (val.waybillStatus !== '待完善') {
        this.$router.push({name: 'WaybillDetail', query: {id: val.waybillId}})
      }
    },
    routerGo () {
      this.$router.back(-1)
    ...
    sure () {
      ...
      //   return false
      // }
      let timeStart = new Date(this.waybillQueryParam.createTimeStart).getTime()
      let timeEnd = new Date(this.waybillQueryParam.createTimeEnd).getTime()
      ...
      let sureFlag = this.validDuringDay(this.waybillQueryParam.createTimeStart, this.waybillQueryParam.createTimeEnd, 180)
      if (sureFlag) {
        setTimeout(() => {
          this.showPop = false
          this.getWaybill(this.waybillQueryParam)
