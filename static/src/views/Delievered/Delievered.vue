<template>
   <div class="main-con">
     <div class="receiveMon" v-show="recSourceList.length === 0 ? false : true">
          ...
          <!-- 待妥投-Bottom Start -->
          <button :class="receiveTotalMoney >0 ? bottomA : bottomB" type="button" :disabled="receiveSelectedList.length === 0" @click="handleToPay()">确认签收</button>
          <!-- 待妥投-Bottom End -->
        </div>
        <!-- 待妥投-无运单 Start -->
        <van-pull-refresh v-model="noBillRecisLoading" @refresh="noBillRecOnRefresh" class="noBill" v-show="recSourceList.length === 0 ? true : false">
          <div class="noBill">
            ...
            <div class="msg">当前没有待妥投的运单，辛苦啦！</div>
          </div>
        </van-pull-refresh>
        <!-- 待妥投-无运单 End -->
        <div v-transfer-dom>
...
export default {
  ...
  methods: {
    ...
    // 获取-待妥投-运单
    getRecWayBill () {
      WayBillAjax.WaitDeliver(this.receiveQueryParam).then(res => {
        if (res.code === 200) {
          this.recSourceList = []
